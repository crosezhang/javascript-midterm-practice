...
When working with the `quote`, we realize many quotes share the same author's name. Therefore, we decide to use closure and currying to make a function that can easily reuse the author's name. Complete the implementation of `curriedQuote`.
...
Here is an example of using `curriedQuote``:
